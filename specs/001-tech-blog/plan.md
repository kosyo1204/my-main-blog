...
Eleventy で Markdown 記事を管理し、フロントマターの `published` により公開/非公開を制御する。
公開記事のみを静的ビルドし、GitHub Actions で Public リポジトリへ成果物を自動配置して
GitHub Pages で公開する。SEO メタ情報と GA4 を組み込み、モバイル閲覧と検索/分類導線を整備する。
...
**制約**: 公開用リポジトリに Markdown 原本を置かない、無料運用、公開/非公開の確実分離  
**規模/スコープ**: 単一筆者、数百記事規模
...
_site/                 # ビルド成果物（Public リポジトリに同期）
.github/workflows/     # GitHub Actions
```

**構成決定**: 既存の Eleventy 構成（content/ と static/）を基盤にし、_site/ を生成物として Public リポジトリへ同期する。
Public リポジトリは別管理で、Markdown 原本は配置しない。
